...
def execute(request):
    ...
    scope,_=active_scope(request['contract_sha256']);stage=scope.directory/'calls'/NONCE
    ...
    argv=argv[:cut]+['--no-alt-screen','-C',str(workspace),prompt]
    ...
    write(stage/'session-exit.json',{'observed_at':datetime.now(timezone.utc).isoformat(),'provider':provider,
        ...
        'interval':'Native independent continuation begins only after this verified actual exit; root sends no technical instruction or continuation signal'})
